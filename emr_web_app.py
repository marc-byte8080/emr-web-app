...
# =========================================================
# CONFIG (must be BEFORE any UI in Streamlit)
# =========================================================
st.set_page_config(
...
DATA_FILE = "emr_data.json"  # must be in same folder as this .py
...
def login_screen():
    ...
    u = st.text_input("Username", placeholder="e.g., nurse", key="login_user")
...
# =========================================================
# CSS (DARK THEME, NO TRANSPARENCY)
# IMPORTANT: DO NOT HIDE stHeader -> keeps the sidebar toggle working
# =========================================================
st.markdown(
    """
<style>
...
/* Keep header visible (so the sidebar toggle works), but make it dark */
header[data-testid="stHeader"]{
  background: #071a2f !important;
  border-bottom: 1px solid #0f2a52 !important;
}

/* Hide Streamlit menu/footer only */
...
/* Collapsed sidebar control (<< >> button area) */
...
def load_records(path: str) -> dict:
    p = Path(path)
    if not p.exists():
        st.error(f"Cannot find data file: {path}\n\nMake sure it is in the SAME folder as emr_web_app.py")
        st.stop()
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except Exception as e:
        st.error(f"Your JSON file exists, but it cannot be read.\n\nError: {e}\n\nTip: JSON must use DOUBLE QUOTES.")
        st.stop()
...
if "group_name" not in st.session_state:
    st.session_state.group_name = "GROUP 3: BSN 2-2"
if "members" not in st.session_state:
    st.session_state.members ="OGALESCO, MARC\ABDUL, SANDRA\PADILLA, JESSICA\SUMAIT, RISHA\TEJERO, MONALISA\YBIOSA, ERICKA"
...
with h1:
    st.markdown('<div class="emr-title">HOSPITAL EMR SYSTEM —<br>ACTIVITY<br>(SIMULATION)</div>', unsafe_allow_html=True)
    st.markdown('<div class="emr-sub">Dashboard • Data loaded from JSON</div>', unsafe_allow_html=True)
...
with h3:
    ...
    export_bytes = json.dumps({st.session_state.selected_pid: current}, indent=2, ensure_ascii=False).encode("utf-8")
    st.download_button(
        "Export JSON",
        data=export_bytes,
...
# =========================================================
# SIDEBAR (patient list + summary + problems + group/members)
# =========================================================
with st.sidebar:
    ...
    st.session_state.members = st.text_area("Members (one per line)", value=st.session_state.members, height=160)
...
with nav_col:
    ...
    st.session_state.section = st.radio(
        "Choose section",
        SECTION_LABELS,
        index=SECTION_LABELS.index(st.session_state.section) if st.session_state.section in SECTION_LABELS else 0,
